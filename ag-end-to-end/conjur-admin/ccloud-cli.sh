...
showUsage() {
  ...
  echo "      $0 [ cert-request <issuer-name>]"
  exit -1
}
...
main() {
  ...
  case $command in
	...
	;;
    cert-request)
	conjur_cert_request $issuerName
	;;
...
function conjur_rotate_api_key {
	...
	local id=$1; shift
	$util_defaults
	api_key=$($CURL $CONJUR_VERBOSE -X PUT  	\
		-H "$authHeader"				\
...
#####################################
function conjur_cert_request {
	local issuerName=$1; shift		# user or host
	$util_defaults
	CERT_REQ="$(< cert-req.json)"
	cert_chain=$($CURL $CONJUR_VERBOSE -X POST  			\
		-H "$authHeader"					\
		-H "Accept: application/x.secretsmgr.v2beta+json"	\
		-H "Content-Type: application/json"			\
		-d "$CERT_REQ"						\
		"$CYBERARK_CCLOUD_API/issuers/${issuerName}/issue")
	echo $cert_chain | jq .
}
